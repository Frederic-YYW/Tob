...
public class DateUtil {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Timestamp ts = new Timestamp(Long.parseLong("1566371585000"));
        Timestamp ts1 = new Timestamp((long) t);
        Date date = new Date();
        Date date1 = new Date();
        date = ts;
        date1 = ts1;
        ...
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
//         time= .parse("1566371585000");
//        String d = format.format(time);
//        Date date2=format.parse(d);

        System.out.println("年 ： " + getYear());
        System.out.println("月 ： " + getDate());
        System.out.println("日 ： " + getDayOfDate(date));
    }

    public static String calLastedTime(long start) {
    ...
    public static String getDiffSecond(int start) {
        LocalDateTime time = LocalDateTime.now();
    ...
    /**
     * 将long类型毫秒值转为秒值，并保留指定小数位
     *
     * @param time
     * @param digit 指定小数位
     * @return
     */
    public static String getSeonds(long time, int digit) {
        return new Formatter().format("%." + digit + "f", (double) (time) / 1000).toString();
    }
     ...
     * @return
     */
    public static Date getSeconds(Date date, int seconds) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
